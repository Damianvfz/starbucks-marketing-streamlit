...
# ============================================================
    ...
    with st.expander("Ver evaluación del número de segmentos RFM"):
        ...
        st.info(
            ...
            "Sin embargo, se utiliza K=3 porque una división en solo dos grupos puede ser demasiado general "
            "para fines comerciales. K=3 permite distinguir clientes de alto valor, valor medio y baja actividad."
        )
    ...
    **Clientes digitales de consumo ampliado**
    ...
    # ========================================================
    # COMPARACIÓN METODOLÓGICA: K-MEANS VS STEPMIX
    # ========================================================

    st.subheader("Comparación metodológica: K-Means vs StepMix")

    st.markdown("""
    Además de K-Means, el grupo evaluó **StepMix** como método alternativo de contraste.
    StepMix permite obtener una segmentación más granular mediante clases latentes, mientras que
    K-Means se mantiene como método principal por su mayor simplicidad e interpretabilidad comercial.
    """)
    ...
    comparacion_metodos = pd.DataFrame({
        ...
        "Método elegido para presentar": [
            "K-Means",
            "K-Means"
        ],
        "Justificación": [
            "Permite distinguir clientes de alto valor, valor medio y baja actividad.",
            "Entrega dos perfiles claros y accionables para una presentación ejecutiva."
        ]
    })

    st.dataframe(comparacion_metodos, use_container_width=True)

    with st.expander("Ver explicación de la decisión metodológica"):
        st.markdown("""
        **K-Means** fue utilizado como modelo principal porque entrega una lectura más simple y defendible
        para una presentación de marketing: permite separar el mercado en grupos claros y accionables.

        **StepMix** fue considerado como método alternativo porque entrega una segmentación más granular.
        En el análisis del grupo, StepMix propone más clases: 5 para RFM y 6 para el modelo socio-demográfico/conductual.

        Sin embargo, una segmentación con demasiados grupos puede dificultar la interpretación y la recomendación final.
        Por eso, se decidió presentar K-Means como modelo principal, manteniendo StepMix como contraste metodológico.
        """)

    st.info(
        "Se evaluaron ambos métodos. StepMix entrega una segmentación más detallada, pero K-Means fue seleccionado "
        "para el análisis final porque permite comunicar segmentos más simples, comparables y accionables para decisiones de marketing."
    )
